...
struct ListView: View {
    @ObservedObject var ridesLoader: RideLoader
    ...
    init(networkService: NetworkService, ref: Int) {
        ...
        self.ref = ref
        ridesLoader = RideLoader(networkService: networkService, ref: ref)
        ...
        ZStack {
            ...
            VStack(alignment: .leading, spacing: 15) {
                ...
                List(ridesLoader.rides) { ride in
                    Rides(ride: ride, networkService: networkService)
                        .zIndex(1)
                        .onTapGesture {
            ...
            ImageDetailsView(isOpen: self.$bottomSheetShown, maxHeight: geometry.size.height * 0.7, ride: selectedRide, networkService: networkService)
...
struct EdvoraList_Previews: PreviewProvider {
    static var previews: some View {
        ListView(networkService: NetworkService(), ref: 20)
    }
}
